...
The range of degrees of freedom is from 1 to the number of unique predictor values, which is `r length(unique(training_x[, "perc.alumni"]))`. Therefore, a range of degrees of freedom is tested, ranging from 10 to 50 by an increment of 10.
...
When not passing a prespecified degree of freedom to `smooth.spline`, it automatically uses generalized cross validation to get the optimal degree of freedom, `r print(ss$df)`, which is used to get the optimal fit for smoothing spline. This is plotted above.
...
When fit to the testing dataset, the GAM has a test error MSE of `r gam_pred_error`.
